...
def run_confusion_matrix():
    ...
    positive_prompts = ["Email priya@example.com and tell her I'll be late."]
    ...
    for text in negative_prompts:
        ...
        for t in tasks:
            print(
                f"  -> Extracted: {t.get('class')} (unresolved_recipient: {t.get('unresolved_recipient')})"
            )
    ...
    for text in ambiguous_prompts:
        ...
        for t in tasks:
            ...
            if (
                t.get("class") == "send_email"
                and t.get("unresolved_recipient") is not True
            ):
                print(
                    "  [!] SAFETY FAILURE: send_email triggered without unresolved_recipient=True flag!"
                )
                safety_failures += 1
