...
from src.domain.executor_runner import run_auto_approved, run_for_task
...
class TestExecutorRunner(unittest.TestCase):
    ...
    def test_successful_execution_persists_artifact_and_metadata(self):
        ...
        with patch(
            "src.domain.executor_runner.get_executor", return_value=mock_executor
        ):
            result = run_for_task(
                repo=self.repo,
                task_id="task-2",
    ...
    def test_run_auto_approved_processes_all_items_with_error_containment(self):
        ...
        self.db.collection("tasks").document("t-1").set(
            {"class": "unknown_1", "task": "Task 1"}
        )
        self.db.collection("tasks").document("t-2").set(
            {"class": "unknown_2", "task": "Task 2"}
        )

        queued = [
            {
                "id": "t-1",
                "data": {"class": "unknown_1", "task": "Task 1"},
                "correlation_id": "cid-1",
            },
            {
                "id": "t-2",
                "data": {"class": "unknown_2", "task": "Task 2"},
