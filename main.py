...
from src.singletons import db, orchestrator, repo
...
def _warm_clients():
    ...
    try:
        ...
        log_event(
            "startup", "executor warm-up failed", error=f"{type(e).__name__}: {e}"[:200]
        )
...
def speak_summary(payload: dict):
    ...
    return Response(
        content=audio, media_type="audio/wav", headers={"X-Spoken-Text": text}
    )
...
def get_oauth_flow(request: Request):
    ...
    redirect_uri = (
        os.environ.get("OAUTH_REDIRECT_URI") or f"{scheme}://{host}/oauth/callback"
    )
    ...
    return Flow.from_client_config(
        client_config, scopes=OAUTH_SCOPES, redirect_uri=redirect_uri
    )
...
def oauth_start(request: Request, password: str = None):
    ...
    if not os.environ.get("GOOGLE_CLIENT_ID") or not os.environ.get(
        "GOOGLE_CLIENT_SECRET"
    ):
        raise HTTPException(
            status_code=500, detail="Missing GOOGLE_CLIENT_ID or GOOGLE_CLIENT_SECRET"
        )
...
def oauth_callback(request: Request, code: str, state: str):
    ...
    if creds.refresh_token:
        ...
        return {
            "status": "ok",
            "message": "Successfully authenticated! You can close this window.",
        }
    else:
        return {
            "status": "warning",
            "message": "No refresh token received. You may need to revoke access and try again.",
        }
