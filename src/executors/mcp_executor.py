            ...
            async with ClientSession(streams[0], streams[1]) as session:
                ...
                response = client.models.generate_content(
                    ...
                    config=types.GenerateContentConfig(
                        tools=gemini_tools if gemini_tools else None, temperature=0.0
                    ),
                )
                ...
                if response.function_calls:
                    for fcall in response.function_calls:
                        args = (
                            fcall.args
                            if isinstance(fcall.args, dict)
                            else dict(fcall.args)
                        )
                        ...
                        result = await session.call_tool(fcall.name, arguments=args)
